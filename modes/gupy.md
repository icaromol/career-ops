...
4. Confirmar que esta é uma vaga Gupy relendo o campo `**URL:**` do header do relatório e checando se o host termina em `.gupy.io` (mesma checagem de `modes/_shared.md` § Platform Detection: Gupy — não faça prose-matching contra o texto da linha `**PDF:**`, que pode ter sido escrita antes desta regra existir, editada manualmente, ou estar em outro idioma). Se a URL não bater com `.gupy.io` — ex: usuário pedindo isso numa vaga não-Gupy, ou relatório sem URL — perguntar uma vez: "Essa vaga não foi marcada como Gupy na avaliação. Quer mesmo assim gerar o texto no formato apresentação Gupy?" Só prosseguir após confirmação.
5. Ler `config/profile.yml` → `ats_profiles.gupy.skills` (lista fixa já cadastrada no perfil Gupy do usuário). Se a chave não existir, parar e avisar o usuário: "Não encontrei `ats_profiles.gupy.skills` em `config/profile.yml` — veja o exemplo comentado em `config/profile.example.yml` para o formato esperado (lista de strings com os nomes exatos das habilidades cadastradas no seu perfil Gupy)." Nunca inventar uma lista de habilidades.
